    ...
    public static double[][][] getProb(double[] seed) {
        ...
        for (int i = 0; i < pixels.length; i++) {
            for (int j = 0; j < pixels[i].length; j++) {
                ...
                for (k = 0; k < hP.length; k++) {
                    for (int l = 0; l < vP.length; l++) {
                        sol[k][l] = flMultiply(sol[k][l], flAdd(flMultiply(new double[]{hP[k], 0}, seed), flMultiply(new double[]{-vP[l], 0}, flAdd(new double[]{-1, 0}, seed))));
                    }
                }
            }
        }
